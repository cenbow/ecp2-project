 ...
 */
@Controller
@RequestMapping("/back/pushmoney-config")
public class PushmoneyConfigController {
	...
	public ModelAndView selectLinkItem(HttpServletRequest request, HttpServletResponse response, Boolean clickPageBtn, PageBean pageBean, String pagehelperFun, Long roleId) {
		...
		if(clickPageBtn!=null && clickPageBtn){
			mav.setViewName(StaticConstants.PUSHMONEY_CONFIG_MANAGE_TABLE_PAGE);
		}else{
			mav.setViewName(StaticConstants.PUSHMONEY_CONFIG_MANAGE_PAGE);
		}
