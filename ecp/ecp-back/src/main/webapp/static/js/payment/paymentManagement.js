...
* @returns
*/
function openModiPaymentCommentDialog() {
	//console.log("debug!");
	$('#modal-container-306692').modal({
		backdrop : 'static',
		keyboard : false
	});
}

/* close dialog :关闭修改回款备注窗口*/
function closeModiPaymentCommentDialog() {	
	$("#modal-container-306692").modal("hide");
}
...
$(function() {
	 ...
	 * 【回款备注管理】按钮
	 */
	$(".edit-payment-comment").on("click", function(e) {
		...
		var url=BASE_CONTEXT_PATH+'/back/payment/loadpaymentcommentdialog';
		var params=new Object();
		params.orderId=orderId;
		params.orderNo=orderNo;
		//动态加载增加dialog
		$(".edit-paymentcomment-dialog").load(url,params,function(){
			//加载完毕后自动打开增加对话框.
			openModiPaymentCommentDialog();
			//当对话框完全关闭后刷新当前页
			$('#modal-container-306692').on('hidden.bs.modal', function () {
				  // 执行一些动作...
				reloadCurrPage();
				})
		});
